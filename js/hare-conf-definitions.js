...
const system = {
  ...
  buftype: {
    ...
    exclude: entry(
      list(T.STR),
      "Buffer types to be excluded in many scenarios.",
      ["nofile", "quickfix", "terminal", "prompt", "help"],
    ),
  },
  filetype: {
...
const appearance = {
  ...
  theme: {
    ...
    mode: entry(
      union("system", "light", "dark"),
      "The mode of using themes.",
      "system",
    ),
...
const editor = {
  ...
  buffer: {
    ...
    treesitter: {
      ...
      name: entry(
        union(T.STR, list(T.STR)),
        "The default Tree-sitter parser to use.",
        null,
      ),
      highlight_enabled: entry(
        T.BOOL,
...
const language = {
  ...
  names: entry(list(T.STR), "A list of strings specific to the language.", [
    "lua",
  ]),
};
...
export const hareConfExtraClasses = {
  "hare.classes.MasonPackageEntry": {
    ...
    executable: entry(
      T.STR,
      "The executable provided by the Mason package.",
      null,
    ),
  },
};
