...
local display_lua_code = function(lua_code, title)
    ...
    if not ok then
        M.error 'nui.nvim is not loaded (required for displaying configuration)'
        return
    end
    ...
    -- The popup window can be closed with `q` or `<Esc>`.
...
local display_buffer_config = function(filetype)
    local is_filetype_given = filetype and filetype ~= ''
    local buffer_config = is_filetype_given and M.fn.get_buffer_config(filetype)
        or M.config.editor.buffer

    display_lua_code(
        vim.inspect(buffer_config),
        is_filetype_given and 'Hare Configuration for Buffer (' .. filetype .. ')'
...
vim.api.nvim_create_user_command(M.COMMAND_NAME, function(opts)
    ...
    elseif subcommand == 'make' then
        ...
        M.warn('Unknown command: ' .. subcommand)
    end
end, {
    nargs = '*',
